...
previous_month <- wv_police_incidents_datetime %>% filter(date >= latest_date$date - 30)%>%
  filter(title=="SUSPICIOUS EVENT")%>%
  summarise(count=n())

sub <- paste("On", format(latest_date$date, format="%B %d"), " ", " A supicious event occured. It was the", format(previous_month),"time it has ocurred in the previous 30 days", sep="")
sub
```
