...
net_migration <- read_csv("tabula-md-statistical-handbook.csv", skip=2, col_names=c("state/jurisdiction", "July_2011", "July_2012", "July_2013", "July_2014", "July_2015","July_2016", "July_2017", "July_2018","July_2019", "total" ))
```
...
net_migration_w_international <- net_migration%>%
...
**A1** The Suburban Washington Region and Montgomery County had the most greatest percentages of Maryland's overall immigration. 

```{r}
# Write the code to produce your answer here
 migration_percent<-net_migration_w_international%>%
...
**A2** The data produced shows the broader regions and specific counties and cities in Maryland by immigration, specifically we created a column which shows the percentage of overall immigration totals for the state that each community or region accounted for. The data shows that the greater Washington region accounts for the most immigration. 
...
**A3** Calvert and the Southern Maryland Region had the largest changes, both seeing significant drops. 

```{r}
# Write the code to produce your answer here
percent_change_2017_2018 <- migration_percent%>%
...
**A4** These are more conservative and less urban areas. They may be more hostile to new immigrants, more homogenous in their demographics, and provide less job opportunities. 
...
```{r}
domestic_migration <- read_csv("table_2b.csv", skip=2, col_names=c("state/jurisdiction", "July_2011", "July_2012", "July_2013", "July_2014", "July_2015","July_2016", "July_2017", "July_2018","July_2019", "total" ))
```
...
```{r}
domestic_migration_w_migration_type <- domestic_migration%>%
...
```{r}
# Write the code to produce your answer here
domestic_migration_w_migration_type%>%
arrange(total)
```
...
# Write the code to produce your answer here
domestic_migration_w_migration_type%>%
  mutate(July_2017 >0 & July_2018 >0 & July_2019 >0)

```

**Q7** How would you describe this data? Is there a county or region that stands out, and why?
**A7** Carroll is interesting because it only has a small turnaround, it is positive for these three years, but it is still down overall. All of the others have overall growth trends and are growing by a much larger amount.
