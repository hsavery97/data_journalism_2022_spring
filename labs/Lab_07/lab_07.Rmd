...
Wyoming which had 25,814 loans for 581,024 inhabitants. 
```{r}
...
big_data_table_dataframe<-big_data_table [[1]]

every_state_ppp<-tibble()

for(row_number in 1:nrow(big_data_table_dataframe)){
  each_row_df<-big_data_table_dataframe%>%
    slice(row_number)
  ...
  specific_state_dataframe<-specific_state[[1]]
  every_state_ppp<-every_state_ppp%>%
  bind_rows(specific_state_dataframe)%>%
    mutate(loans_per_hundred_thousand = (total_ppp_loans/population)*100)%>%
    arrange(
      desc(loans_per_hundred_thousand)
    )
}
