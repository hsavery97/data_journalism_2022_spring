...
ppp_wv_loans_April <- wv_ppp_loan_data %>%
  filter(date_approved > "2020-3-31" & date_approved < "2020-5-01")%>%
...
ppp_wv_loans_April %>%
 ...
 labs(
    title="Which day in April 2020 saw the most loans approved in West Virginia?",
    x = "date",
    y = "total loans",
    caption = "source: SBA PPP loan database")+
geom_point(aes(x=date_approved, y=total_loans))+
geom_text(aes(x=date_approved, y=total_loans, label=total_loans), size=3, hjust=1.5)
...
```{r}
total_ppp_by_state<-read_csv("data/state_ppp_all.csv")

total_ppp_by_state<-total_ppp_by_state%>%
...
all_states <- get_acs(geography = "state",
              variables = c(total_pop = "B01001_001"),
              year = 2019)%>%
rename(state=NAME)

all_states <- all_states%>% arrange(desc(estimate)) 



PPP_pop <- all_states %>% inner_join(total_ppp_by_state)%>%
select(-GEOID, -variable, -moe)%>%
 mutate(per_hundredK = (total_ppp_loans/estimate)*100000)%>%
    arrange(
      desc(per_hundredK
    ))%>%
head(10)
...
PPP_pop %>%
  ggplot() +
  geom_bar(aes(x=reorder(state,per_hundredK), weight=per_hundredK), fill="blue") +
  coord_flip() + 
    labs(
      title="States With Most PPP Loans Per 100k People",
      x = "States",
      y = "Loans per $100,000",
      caption = "Source: SBA PPP Loan Database"
    )
...
South Dakota, North Dakota, Nebraska, Georgia, Iowa, Illionois, Louisiana, Florida, Mississippi, Montana, in that order.
...
**A3.**  
Most loans were forgiven for about as much as they were approved for, there is more variability at the top end of the spectrum, but in general most loans were forgiven for their approval amount.

```{r}
wv_ppp_loan_data <- read_rds("data/lab_09.rds")

wv_loans_low<-wv_ppp_loan_data%>%
  filter(amount < 25000)%>%
   drop_na(forgiveness_amount)
