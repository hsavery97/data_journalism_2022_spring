...
**Answer**
A dataframe of covid tweets by elected officials.
```{r}
...
**Answer**
It pulls out indiviudal words from tweets.
...
**Answer**
Not really, the top word is the.
...
**Answer**
Removes stop words, no more the's and and's.
```{r}
unique_words_2020 %>%
  anti_join(stop_words) %>%
  group_by(word) %>%
...
**Task** Run the following code and describe the results.
**Answer**
covid, pandemic, etc. More context than one word phrases though
```{r}
covid_tweets %>%
  filter(created < '2021-01-01') %>%
...
**Task** Run the following code and describe the results.
**Answer**
Mostly variants of covid and healthcare.
...
**Task** Run the following code and describe the results.
**Answer**
A lot fewer republican tweets about covid and more of an emphasis on helathcare for Democrats.
```{r}
covid_tweets %>%
...
**Task** Run the following code and describe the results.
**Answer**
We see Republican language shift between year things like illegal immigrants start to enter.
```{r}
covid_tweets %>%
...
**Answer**
positive is positive, in this context it should probably be negative.
