...
**Answer**
It's counting the total number of laons per county and then organziing those from greatest to least in a new data frame.
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer**
A bar chart, it's a little messy tbh.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
...
**Answer**
It is now grouped in ascending order.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
The axis flips, much more legible
...
**Answer**
There is a blue background now. Looks much more attractive.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
Better labeled, has a title and a caption.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
Line chart, crazy looking through kind of looks like a heart attack.Shows a spike and drop off and then resurgence in loan approvals.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
Dates are now given on a slant, much more legible.
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
