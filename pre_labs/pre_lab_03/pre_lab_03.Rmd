...
**Answer**:135073		
They're the same, conclude that all roads lead to rome.
...
**Answer**
No data for October, between rounds of loans.
...
**Answer**
Largest: 10000000 Smallest: 6 I'm very suspicious of the $6 loan, who is that keeping on the payroll, that isnt even minimum wage.
```{r}
...
**Answer**
Inconsistent spelling and capitalization of city names, inconsistent zip code length, duplicate entries
...
**Answer**
The columns are all lowercase now and don't have any spaces.
...
**Answer**
There is an error because amount is lsited as a character.
...
**Answer**
one duplicate, STUART JONES LLC
```{r}
cleaned_arnold_md_loans %>%
  get_dupes()
```
...
**Answer**
Note sure what the question is here, but it worked!
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
